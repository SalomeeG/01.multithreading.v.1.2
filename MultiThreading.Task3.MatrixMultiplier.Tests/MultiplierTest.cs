...
namespace MultiThreading.Task3.MatrixMultiplier.Tests
{
    ...
    public class MultiplierTest
    {
        ...
        public void ParallelEfficiencyTest()
        {
            ...
            var timeOfRegular = TimeSpan.Zero;
            var timeOfParallel = TimeSpan.Zero;
            ...
            multiplierParallel.Multiply(new Matrix(1, 1), new Matrix(1, 1));
            multiplier.Multiply(new Matrix(1, 1), new Matrix(1, 1));

            while (timeOfRegular <= timeOfParallel && sizeOfMatrix <= byte.MaxValue)
            {
                ...
                sw.Stop();
                timeOfRegular = sw.Elapsed;

                sw.Reset();

                sw.Start();
                multiplierParallel.Multiply(firstMatrix, secondMatrix);
                sw.Stop();
                timeOfParallel = sw.Elapsed;

                sizeOfMatrix++;
            }

            Assert.IsTrue(timeOfRegular > timeOfParallel);
            System.Diagnostics.Trace.WriteLine($"Time Of Regular {timeOfRegular}");
            System.Diagnostics.Trace.WriteLine($"Time Of Parallel {timeOfParallel}");
            System.Diagnostics.Trace.WriteLine($"Size Of Matrix {sizeOfMatrix}");
        }
